...
- **Campo estadístico (SBF):** la luz de las estrellas que el sistema NO separa, tratada como media más fluctuación en vez de como estrellas dibujadas. Su brillo sale de `S1(m_lim)` (primer momento de la LF en flujo) y su granulado de `S2(m_lim)` (segundo momento). El contraste del grano es consecuencia de la LF, nunca un parámetro. Se pinta como **lognormal** de esa media y esa varianza, no como gaussiana: donde hay menos de una estrella no resuelta por beam σ supera a ⟨I⟩ y una gaussiana recortada a I ≥ 0 se inventa el flujo que le falta. _Evitar_: paquete, halo, textura.
