  ...
  function arrancar() {
    try {
      ...
      function crearTarjeta(obs) {
        ...
        var acciones = obs.mia ? botonesDe(obs) : '<span class="not-mine">de otro observador</span>';
      ...
      function botonesDe(obs) {
        if (viendoPapelera) {
          return '<button type="button" class="act restore" data-accion="restaurar">Restaurar</button>';
        }
      ...
      function conectarAcciones(card, obs) {
        var btn = card.querySelector('[data-accion]');
        if (!btn) return;
