...
- **El término FÍSICO no depende de la magnitud**: el disco lo fijan apertura, aumento y seeing, y es el mismo para todas las estrellas del campo. Quien engorda a las brillantes es el suelo (`radioSueloMag`/`radioSueloMax`, sobre el flujo absoluto `(D/200)²·10^(-0,4·g)`, con tope), y eso es **convención de atlas, no física**. Está acotado a 3,0× entre una brillante saturada y una del límite (sección 11 de `scripts/test_estrella_fisica.js`): con la escala anterior (3,8×) las brillantes salían como bolas, y ese mismo engorde era lo que se comía el hueco de los pares apretados —Almaak sumaba 5,5 px de discos contra 4,5 px de hueco—, que hoy protege aparte el recorte por `sep`. El brillo lo cuentan opacidad, glow, spikes y curva de tono, que además ensancha los núcleos saturados.
...
- **Un paso = 10 % del campo real DIBUJADO.** El `arcmin` que come el render, no el que pide el ocular: en la placa del DSS viene recortado a 3° (2° si entra el ESO de respaldo) y el paso tiene que valer lo que se ve. `Δ = 0,10 · arcmin / 60` grados.
