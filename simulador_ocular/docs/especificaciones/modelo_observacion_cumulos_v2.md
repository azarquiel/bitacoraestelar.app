...
- **v7 E0 — hecha** (`scripts/harness_halo_v7.js`, `scripts/test_harness_halo_v7.js`, 11 comprobaciones; volcado en `simulador_ocular/docs/validacion/e0_referencia.txt`). Cero diffs en producción. Los tres defectos, medidos: **D1**, a 146× el difuso llega a 3,89 r_h en M13 y 4,50 r_h en 47 Tuc, unas 4,4 mag por debajo del cielo y aún encendido. **D3**, la segunda diferencia de μ(r) vale 0,003-0,04 mag sobre el King puro y 2,5-57× más sobre ⟨I⟩ = Σ·S1 en la misma malla: **el codo no lo mete ni el muestreo radial ni la cuantización a 8 bits, sino `S1(m_res)` saltando de bin en bin de la LF mientras `m_res(r)` los barre** —`mCrowd` interpola dentro del bin, `S1`/`S2` no—. Es la rama que implementa E4, y no es ninguna de las dos que el documento de tareas anticipaba.
...
- **v7 E3 — cerrada SIN cambio en producción** (`scripts/test_halo_v7_e3.js`, 13 comprobaciones). El truncamiento de King y su normalización **ya estaban** en el código desde la Fase 2: `perfilKing` lleva el término `((a − b)/v0)²` con `b = 1/√(1 + (r_t/r_c)²)`, y `areaKing` es su integral cerrada hasta `r_t`, que es con lo que `poblacionCacheada` normaliza Σ. Lo que faltaba era la prueba:
  ...
  4. **Las alas se apagan solas a 2,8·r_h**, muy dentro del 4·r_h exigido. En la captura de D1 (M13, 200 mm, 146×, SQM 21,5) el contraste máximo del ala más allá de 4·r_h es **0,150 frente a un Cmin de 0,491**, y el tap perceptual no pinta nada en ninguno de los 12 anillos medidos. El contraste se mide como ⟨I⟩/F_cielo, los dos SIN atenuar, que es el marco en el que trabaja el render (E1.3). **D1 no lo causa la extensión del perfil.**

- **v7 E4 — CON cambio en producción** (`scripts/test_halo_v7_e4.js`, 15 comprobaciones). **D3 (los anillos concéntricos de 47 Tuc) lo causaba la cola de la LF, no el perfil.** `S1(m_lim)` y `S2(m_lim)` eran sumas de cola por bins y devolvían **el bin entero**: funciones escalón de `m_lim`. Como `m_res(r)` sí es continua en el radio —`m_crowd` interpola dentro del bin justo por el invariante 7—, cada borde de bin que `m_res` cruzaba metía en ⟨I⟩(r) = Σ(r)·S1(m_res+δ) un salto de un bin de LF. Medido antes del arreglo: **0,18 mag en 47 Tuc (r = 223″), 0,19 en M13 (r = 79″) y en ω Cen (r = 431″)**, entre nodos vecinos de la tabla. Un 18 % de salto de brillo en un anillo se ve a simple vista.
  1. **El arreglo es una función, `cola(tabla, mlim)`**, que reparte linealmente el bin que `m_lim` parte —tratar φ como densidad constante dentro del bin, que es la hipótesis con la que la LF está tabulada—. `S1`, `S2` y `Fresuelto` pasan por ella. **No hay constante nueva y ninguna de H2c se toca.** Residuo tras el arreglo: **0,033 / 0,044 / 0,024 mag**, entre 4,4 y 8 veces menor, y ya proporcional al movimiento de `m_res` (cociente máximo **0,84 mag de cola por mag de límite**, contra 4,3 con la cola escalonada: eso es la LF, no un escalón).
  2. **La fotometría no se mueve.** En los bordes de bin la cola interpolada coincide con la suma directa **al 1e-15**, dentro del bin queda **entre los dos bins enteros que la rodean**, `S1 + Fresuelto = F(V_t)` sigue exacto y S1 completa sigue siendo el flujo total. El escalón se va sin que se pierda ni se duplique un fotón.
  3. **Queda un codo de segunda derivada**, no un escalón: la cola interpolada es C0 pero no C1 (la pendiente cambia al cruzar el borde de bin). Se descartó suavizarla más —interpolar φ linealmente entre centros de bin daría C1— porque la integral trapezoidal deja medio bin fuera en cada extremo y **eso sí movería la fotometría**, que es exactamente lo que v7 prohíbe.
  4. **`scripts/test_cumulos.js` §3 hubo que actualizarlo**: codificaba la semántica escalón («bins con centro > m_lim»). Ahora comprueba el contrato nuevo, que es más fuerte: exacto en los bordes y encajonado entre los dos bins en medio.
